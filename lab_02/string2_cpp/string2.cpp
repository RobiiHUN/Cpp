 ...
 */
 String addString(const String& s0, const String& s1){
    String s2;
    s2.len = s1.len + s0.len;
    s2.pData = new char[s2.len + 1]; 
    s2.pData = strcpy(s2.pData, s0.pData);
    s2.pData = strcat(s2.pData, s1.pData);
    s2.pData[s2.len] = '\0';
    return s2;
...
String addString(const String& s0, char ch){
    ...
    s2.len = s0.len + 1;
    s2.pData = new char[s2.len + 1];
    strcpy(s2.pData, s0.pData);
    s2.pData[s2.len -1 ] = ch;
    s2.pData[s2.len] = '\0';
    return s2;
